<x-app-layout>
    <div class="container mx-auto mt-8 p-4 bg-white rounded shadow">
        ...
        @foreach ($order->orderItems as $item)
            <p>{{ $item->quantity }} x {{ $item->product->name }} - Rp. {{ number_format($item->price, 2) }}</p>
        @endforeach

        <div class="mt-6 text-right">
            <p class="text-xl font-semibold mb-2">Total Price: Rp. {{ number_format($order->total_price, 2) }}</p>
